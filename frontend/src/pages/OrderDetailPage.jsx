...
function OrderDetailPage() {
    ...
    const fetchPedidoDetails = useCallback(async () => {
        try {
            const response = await fetchWithAuth(`${API_BASE_URL}/pedidos/${pedidoId}`);
    ...
    if (!pedido) return <p>Pedido não encontrado ou você não tem permissão para vê-lo.</p>;
                ...
                <ul>
                    {pedido.itens.map(item => (
                        <li key={item.id}>
                            ...
                            <span>R$ {(Number(item.preco_unitario) * item.quantity).toFixed(2).replace('.', ',')}</span>
                        </li>
                    ))}
                </ul>
