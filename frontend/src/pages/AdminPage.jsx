// Arquivo: frontend/src/pages/AdminPage.jsx (Com status de Retirada)
...
function AdminPage() {
  ...
  const fetchAdminData = useCallback(async () => {
    ...
    try {
      ...
      const [pRes, peRes, cRes, bRes] = await Promise.all([
        fetch(`${API_BASE_URL}/produtos`),
        fetch(`${API_BASE_URL}/pedidos`, { headers }),
        fetch(`${API_BASE_URL}/categorias`),
        fetch(`${API_BASE_URL}/banners`, { headers })
      ]);
      if (!pRes.ok || !peRes.ok || !cRes.ok || !bRes.ok) throw new Error('Falha ao carregar dados.');
  ...
  return (
    <div className="admin-container">
      ...
      <div className="admin-section"> <div className="order-management-section"> <h2>Pedidos Recebidos</h2> <div className="order-list"> {pedidos.length > 0 ? pedidos.map(pedido => ( <div key={pedido.id} className="order-card"> <div className="order-card-header"> <h4>Pedido #{pedido.id}</h4> <div className="order-status-control">
      ...
      </div> </div> <div className="order-card-body"> <p><strong>Cliente:</strong> {pedido.nome_cliente}</p> <p><strong>Endereço:</strong> {pedido.endereco_cliente}</p> <p><strong>Itens:</strong></p> <ul> {pedido.itens?.map(item => ( <li key={item.id}> {item.quantidade}x {item.produto_nome || `Produto ID ${item.produto_id}`}</li> ))} </ul> </div> </div> )) : <p>Nenhum pedido.</p>} </div> </div> </div>
    </div>
  );
}
