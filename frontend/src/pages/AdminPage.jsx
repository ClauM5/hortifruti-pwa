// Arquivo: frontend/src/pages/AdminPage.jsx (Com Gerenciador de Categorias)
...
function AdminPage() {
  ...
  const [senha, setSenha] = useState('102030'); // << SENHA JÁ INSERIDA
  ...
  const fetchAdminData = useCallback(async () => {
    ...
    try {
      ...
      const [produtosResponse, pedidosResponse, categoriasResponse] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}/categorias`) // << BUSCA AS CATEGORIAS TAMBÉM
      ]);
  ...
  // ... (handleStatusChange, handleImageUpload, handleDelete continuam os mesmos)...
  ...
  // << LÓGICA CRUD PARA PRODUTOS ATUALIZADA >>
  const handleSaveProduto = async (e) => {
    ...
    // Pega apenas os IDs das categorias selecionadas
    const categoriaIds = formProduto.categorias.map(cat => cat.id);
    const produtoData = { ...formProduto, imagem: imageUrl, categorias: categoriaIds };
    ...
    try {
      const response = await fetch(url, { method: method, headers: { 'Content-Type': 'application/json', 'authorization': "102030" }, body: JSON.stringify(data) });
  ...
  // << NOVAS FUNÇÕES CRUD PARA CATEGORIAS >>
  const handleSaveCategoria = async (e) => {
    ...
    try {
        await fetch(url, { method, headers: { 'Content-Type': 'application/json', 'authorization': "102030" }, body: JSON.stringify({ nome }) });
        setFormCategoria({ id: null, nome: '' });
        fetchAdminData();
    } catch (err) { setError('Falha ao salvar categoria.'); }
  };
  const handleDeleteCategoria = async (catId) => {
    if (!window.confirm("Certeza? Deletar uma categoria a removerá de todos os produtos.")) return;
    try {
        await fetch(`${API_BASE_URL}/categorias/${catId}`, { method: 'DELETE', headers: { 'authorization': "102030" } });
        fetchAdminData();
    } catch (err) { setError('Falha ao deletar categoria.'); }
  };

  // --- Funções Auxiliares ---
  const handleEditProduto = (produto) => {
    // Garante que o produto tenha um array de categorias, mesmo que vazio
    const categoriasDoProduto = categorias.filter(cat => produto.categorias?.includes(cat.nome));
    setFormProduto({ ...produto, categorias: categoriasDoProduto });
    setImagemFile(null);
    window.scrollTo(0, 0);
  };
  ...
  // << NOVA: lida com a mudança das checkboxes de categoria >>
  const handleCategoryChange = (categoria) => {
    setFormProduto(prevForm => {
        const CategoriaExiste = prevForm.categorias.find(cat => cat.id === categoria.id);
        if (CategoriaExiste) {
            return { ...prevForm, categorias: prevForm.categorias.filter(cat => cat.id !== categoria.id) };
        } else {
            return { ...prevForm, categorias: [...prevForm.categorias, categoria] };
        }
    });
  };
  ...
  useEffect(() => {
    if (isLoggedIn) {
      ...
      const ws = new WebSocket(WS_URL.replace('https', 'wss'));
    ...
    <div className="admin-container">
      ...
      <div className="category-manager">
        ...
        <form onSubmit={handleSaveCategoria} className="category-form">
          <input type="text" placeholder="Nome da nova categoria" value={formCategoria.nome} onChange={(e) => setFormCategoria({ ...formCategoria, nome: e.target.value })} />
          <button type="submit" className="admin-button">Salvar Categoria</button>
        </form>
        <div className="category-list">
          {categorias.map(cat => (
            <div key={cat.id} className="category-item">
              <span>{cat.nome}</span>
              <button onClick={() => handleDeleteCategoria(cat.id)} className="delete-button-small">X</button>
            </div>
          ))}
        </div>
      </div>
      
      {/* Formulário de Produtos ATUALIZADO */}
      <form onSubmit={handleSaveProduto} className="admin-form">
        <h3>{formProduto.id ? 'Editando Produto' : 'Adicionar Novo Produto'}</h3>
        {/* ... (inputs de nome, preço, unidade e imagem continuam os mesmos) ... */}
        ...
        <label htmlFor="imagem-upload">Imagem do Produto:</label> <input id="imagem-upload" type="file" name="imagem" onChange={handleFileChange} accept="image/*" /> 
        ...
        <div className="category-checkboxes">
          ...
          {categorias.map(cat => (
            <label key={cat.id}>
              <input type="checkbox" checked={formProduto.categorias.some(formCat => formCat.id === cat.id)} onChange={() => handleCategoryChange(cat)} />
              {cat.nome}
            </label>
          ))}
        </div>

        <div className="form-buttons"> <button type="submit" className="admin-button save-button" disabled={isUploading}> {isUploading ? 'Enviando...' : 'Salvar'} </button> {formProduto.id && <button type="button" onClick={clearFormProduto} className="admin-button clear-button">Cancelar Edição</button>} </div> </form>
      
      {/* ... (Listas de Produtos e Pedidos continuam as mesmas) ... */}
