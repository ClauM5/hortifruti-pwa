// Arquivo: frontend/src/pages/AdminPage.jsx (Versão Final Completa)
...
function AdminPage() {
  ...
  const fetchAdminData = useCallback(async () => {
    ...
    try {
      ...
      const [pRes, peRes, cRes, bRes, cupRes] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}/cupons`, { headers })
      ]);
      if (!pRes.ok || !peRes.ok || !cRes.ok || !bRes.ok || !cupRes.ok) throw new Error('Falha ao carregar dados do admin.');
      setProdutos(await pRes.json());
      setPedidos(await peRes.json());
      setCategorias(await cRes.json());
      setBanners(await bRes.json());
      setCupons(await cupRes.json());
  ...
  useEffect(() => {
    ...
    ws.onopen = () => console.log('Admin conectado ao WebSocket.');
    ws.onmessage = (event) => {
  ...
  const handleStatusChange = useCallback(async (pedidoId, novoStatus) => { try { await fetch(`${API_BASE_URL}/pedidos/${pedidoId}/status`, { method: 'PUT', headers: { 'Content-Type': 'application/json', 'authorization': "102030" }, body: JSON.stringify({ status: novoStatus }) }); setPedidos(prev => prev.map(p => p.id === pedidoId ? { ...p, status: novoStatus } : p)); } catch (err) { setError(err.message); } }, []);
  ...
  const handleSaveProduto = async (e) => { e.preventDefault(); setIsLoading(true); let imageUrl = formProduto.imagem; if (imagemFile) { imageUrl = await handleImageUpload(imagemFile); if (!imageUrl) { setIsLoading(false); return; } } const categoriaIds = formProduto.categorias.map(cat => cat.id); const produtoData = { ...formProduto, imagem: imageUrl, categorias: categoriaIds }; const { id, ...data } = produtoData; const method = id ? 'PUT' : 'POST'; const url = id ? `${API_BASE_URL}/produtos/${id}` : `${API_BASE_URL}/produtos`; try { await fetch(url, { method, headers: { 'Content-Type': 'application/json', 'authorization': "102030" }, body: JSON.stringify(data) }); clearFormProduto(); fetchAdminData(); } catch (err) { setError(err.message); } finally { setIsLoading(false); } };
  const handleDeleteProduto = async (produtoId) => { if (!window.confirm("Certeza?")) return; try { await fetch(`${API_BASE_URL}/produtos/${produtoId}`, { method: 'DELETE', headers: { 'authorization': "102030" } }); fetchAdminData(); } catch (err) { setError(err.message); } };
  ...
  const handleSaveCupom = async (e) => { e.preventDefault(); setError(''); const cupomData = { ...formCupom, usos_maximos: formCupom.usos_maximos || null, data_validade: formCupom.data_validade || null, valor: Number(formCupom.valor) }; const { id, ...data } = cupomData; const method = id ? 'PUT' : 'POST'; const url = id ? `${API_BASE_URL}/cupons/${id}` : `${API_BASE_URL}/cupons`; try { const response = await fetch(url, { method, headers: {'Content-Type': 'application/json', 'authorization': '102030'}, body: JSON.stringify(data) }); if (!response.ok) { const errData = await response.json(); throw new Error(errData.message || 'Falha ao salvar cupom.'); } clearFormCupom(); fetchAdminData(); } catch(err) { setError(err.message); } };
  const handleDeleteCupom = async (cupomId) => { if (!window.confirm("Certeza?")) return; try { await fetch(`${API_BASE_URL}/cupons/${cupomId}`, { method: 'DELETE', headers: { 'authorization': '102030' } }); fetchAdminData(); } catch (err) { setError('Falha ao deletar cupom.'); } };
  ...
  const handleCupomFormChange = (e) => { const { name, value, type, checked } = e.target; setFormCupom(prev => ({ ...prev, [name]: type === 'checkbox' ? checked : value })); };
  const handleEditCupom = (cupom) => setFormCupom({ ...cupom, data_validade: cupom.data_validade ? cupom.data_validade.split('T')[0] : '', usos_maximos: cupom.usos_maximos || '' });
  const clearFormCupom = () => setFormCupom({ id: null, codigo: '', tipo_desconto: 'PERCENTUAL', valor: '', data_validade: '', ativo: true, usos_maximos: '' });
      ...
      <div className="admin-section">
        ...
        <form onSubmit={handleSaveCupom} className="admin-form">
            ...
            <div className="form-grid">
              ...
              <select name="tipo_desconto" value={formCupom.tipo_desconto} onChange={handleCupomFormChange}> <option value="PERCENTUAL">Percentual (%)</option> <option value="FIXO">Valor Fixo (R$)</option> </select>
              <input type="date" name="data_validade" value={formCupom.data_validade} onChange={handleCupomFormChange} />
              <input type="number" name="usos_maximos" placeholder="Usos Máximos (opcional)" value={formCupom.usos_maximos || ''} onChange={handleCupomFormChange} />
            </div>
            <label><input type="checkbox" name="ativo" checked={formCupom.ativo} onChange={handleCupomFormChange} /> Ativo?</label>
            <div className="form-buttons"> <button type="submit" className="admin-button save-button">Salvar Cupom</button> {formCupom.id && <button type="button" onClick={clearFormCupom} className="admin-button clear-button">Cancelar Edição</button>} </div>
        </form>
        <div className="coupon-list"> {cupons.map(cupom => ( <div key={cupom.id} className="list-item"> <span><strong>{cupom.codigo}</strong></span> <span>{cupom.tipo_desconto === 'PERCENTUAL' ? `${cupom.valor}% OFF` : `R$${cupom.valor} OFF`}</span> <span>Usos: {cupom.usos_atuais || 0} / {cupom.usos_maximos || '∞'}</span> <span className={cupom.ativo ? 'status-active' : 'status-inactive'}>{cupom.ativo ? 'Ativo' : 'Inativo'}</span> <div className="item-buttons"> <button onClick={() => handleEditCupom(cupom)} className="admin-button edit-button">Editar</button> <button onClick={() => handleDeleteCupom(cupom.id)} className="admin-button delete-button">Deletar</button> </div> </div> ))} </div>
      </div>
      
      <div className="admin-section">
        <h3>Gerenciar Banners</h3>
        <form onSubmit={handleSaveBanner} className="admin-form">
          <h4>{formBanner.id ? 'Editando Banner' : 'Novo Banner'}</h4>
          <input type="text" name="titulo" placeholder="Título" value={formBanner.titulo} onChange={handleBannerFormChange} />
          ...
          <label>Imagem: <input type="file" onChange={handleBannerFileChange} accept="image/*" /></label>
          <div className="form-buttons"> <button type="submit" className="admin-button save-button" disabled={isUploading}>Salvar Banner</button> {formBanner.id && <button type="button" onClick={clearFormBanner} className="admin-button clear-button">Cancelar</button>} </div>
        </form>
        <div className="banner-list"> {banners.map(b => ( <div key={b.id} className="banner-item"> <img src={b.imagem_url} alt={b.titulo || 'Banner'} className="item-thumbnail"/> <span>{b.titulo || 'Banner'} (Ordem: {b.ordem})</span> <span className={b.is_active ? 'status-active' : 'status-inactive'}>{b.is_active ? 'Ativo' : 'Inativo'}</span> <div className="item-buttons"> <button onClick={() => handleEditBanner(b)} className="admin-button edit-button">Editar</button> <button onClick={() => handleDeleteBanner(b.id)} className="admin-button delete-button">Deletar</button> </div> </div> ))} </div>
      </div>

      <div className="admin-section">
        <div className="category-manager">
          <h3>Gerenciar Categorias</h3>
          <form onSubmit={handleSaveCategoria} className="category-form"> <input type="text" placeholder="Nome da categoria" value={formCategoria.nome} onChange={(e) => setFormCategoria({ ...formCategoria, nome: e.target.value })} /> <button type="submit" className="admin-button">Salvar</button> </form>
          <div className="category-list"> {categorias.map(cat => ( <div key={cat.id} className="category-item"> <span>{cat.nome}</span> <button onClick={() => handleDeleteCategoria(cat.id)} className="delete-button-small">X</button> </div> ))} </div>
        </div>
      </div>
      
      <div className="admin-section">
        <form onSubmit={handleSaveProduto} className="admin-form">
          <h3>{formProduto.id ? 'Editando Produto' : 'Novo Produto'}</h3>
          <input type="text" name="nome" placeholder="Nome" value={formProduto.nome} onChange={handleFormChange} required />
          <input type="number" name="preco" placeholder="Preço" value={formProduto.preco} onChange={handleFormChange} required step="0.01" />
          <input type="text" name="unidade" placeholder="Unidade" value={formProduto.unidade} onChange={handleFormChange} required />
          <label>Imagem:</label> <input type="file" onChange={handleFileChange} accept="image/*" />
          <div className="category-checkboxes"> <label>Categorias:</label> {categorias.map(cat => ( <label key={cat.id}> <input type="checkbox" checked={formProduto.categorias.some(c => c.id === cat.id)} onChange={() => handleCategoryChange(cat)} /> {cat.nome} </label> ))} </div>
          <div className="form-buttons"> <button type="submit" className="admin-button save-button" disabled={isUploading}>Salvar Produto</button> {formProduto.id && <button type="button" onClick={clearFormProduto} className="admin-button clear-button">Cancelar</button>} </div>
        </form>
        <div className="product-list"> {produtos.map(p => ( <div key={p.id} className="product-item"> <img src={p.imagem || 'https://via.placeholder.com/50'} alt={p.nome} className="item-thumbnail" /> <span>{p.nome} - R$ {Number(p.preco).toFixed(2)}</span> <div className="item-buttons"> <button onClick={() => handleEditProduto(p)} className="admin-button edit-button">Editar</button> <button onClick={() => handleDeleteProduto(p.id)} className="admin-button delete-button">Deletar</button> </div> </div> ))} </div>
      </div>
      
      <div className="admin-section">
        <div className="order-management-section">
          <h2>Pedidos Recebidos</h2>
          <div className="order-list"> {pedidos.length > 0 ? pedidos.map(pedido => ( <div key={pedido.id} className="order-card"> <div className="order-card-header"> <h4>Pedido #{pedido.id}</h4> <div className="order-status-control"> <select value={pedido.status} onChange={(e) => handleStatusChange(pedido.id, e.target.value)}> <option value="Recebido">Recebido</option> <option value="Em Preparo">Em Preparo</option> <option value="Pronto para retirada">Pronto para retirada</option> <option value="Saiu para Entrega">Saiu para Entrega</option> <option value="Entregue">Entregue</option> <option value="Cancelado">Cancelado</option> </select> </div> </div> <div className="order-card-body"> <p><strong>Cliente:</strong> {pedido.nome_cliente}</p> <p><strong>Itens:</strong></p> <ul> {pedido.itens?.map(item => { const produtoInfo = produtos.find(p => p.id === item.produto_id); const produtoNome = produtoInfo ? produtoInfo.nome : `Produto ID ${item.produto_id}`; return ( <li key={item.id}> {item.quantidade}x {produtoNome}</li> ); })} </ul> </div> </div> )) : <p>Nenhum pedido.</p>} </div>
        </div>
      </div>
    </div>
  );
