// Arquivo: frontend/src/pages/AdminPage.jsx (Versão com useEffect estabilizado)

import React, { useState, useEffect, useRef, useCallback } from 'react'; // 1. Importa o useCallback
import './AdminPage.css';

// ... (constantes de URL continuam as mesmas)
...
function AdminPage() {
  // ... (todos os useStates continuam os mesmos)
  ...
  // 2. "Estabilizamos" a função com useCallback
  ...
  const handleStatusChange = useCallback(async (pedidoId, novoStatus) => {
    setError('');
    try {
      const response = await fetch(`${API_PEDIDOS_URL}/${pedidoId}/status`, {
        method: 'PUT',
        headers: {
      ...
      // Em vez de recarregar tudo, apenas atualizamos o pedido localmente
      setPedidos(prevPedidos => prevPedidos.map(p => p.id === pedidoId ? { ...p, status: novoStatus } : p));

    } catch (err) {
      setError(err.message);
    }
  }, []); // Adicionado useCallback para consistência


  // 3. Juntamos toda a lógica de "setup" em um único useEffect
  useEffect(() => {
    if (!isLoggedIn) return;

    // Ação 1: Buscar os dados iniciais
    fetchAdminData();

    // Ação 2: Configurar o WebSocket
    ...
    // Ação 3: Função de limpeza
    return () => {
      ws.close();
    };
  }, [isLoggedIn, fetchAdminData]); // O efeito depende de isLoggedIn e da função fetchAdminData


  // ... (o resto do código, como as funções de CRUD e a renderização, continua o mesmo)
