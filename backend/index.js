...
app.get('/api/admin/stats', checkPassword, async (req, res) => { const { range = 'total' } = req.query; let whereClause = ''; switch (range) { case 'hoje': whereClause = "WHERE data_pedido >= current_date"; break; case 'semana': whereClause = "WHERE data_pedido >= date_trunc('week', current_date)"; break; case 'mes': whereClause = "WHERE data_pedido >= date_trunc('month', current_date)"; break; case 'ano': whereClause = "WHERE data_pedido >= date_trunc('year', current_date)"; break; } try { const totalPedidosResult = await pool.query(`SELECT COUNT(*) FROM pedidos ${whereClause}`); const receitaResult = await pool.query(`SELECT SUM(valor_total) FROM pedidos WHERE status = 'Entregue' ${whereClause ? 'AND' + whereClause.substring(5) : ''}`); const pendentesResult = await pool.query(`SELECT COUNT(*) FROM pedidos WHERE status NOT IN ('Entregue', 'Cancelado') ${whereClause ? 'AND' + whereClause.substring(5) : ''}`); const ultimosPedidosResult = await pool.query('SELECT id, nome_cliente, valor_total, status FROM pedidos ORDER BY data_pedido DESC LIMIT 5'); res.json({ totalPedidos: totalPedidosResult.rows[0].count || 0, receitaTotal: receitaResult.rows[0].sum || 0, pedidosPendentes: pendentesResult.rows[0].count || 0, ultimosPedidos: ultimosPedidosResult.rows, }); } catch (error) { res.status(500).json({ message: 'Erro ao buscar estatísticas.' }); }});
...
async function sendPushNotification(userId, messagePayload) {
    ...
    try {
        ...
        if (error.statusCode === 410) { await pool.query('DELETE FROM notificacao_subscriptions WHERE usuario_id = $1', [userId]); } 
        else { console.error('Erro ao enviar push notification:', error); }
    }
}
