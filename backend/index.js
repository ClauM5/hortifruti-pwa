// Arquivo: backend/index.js (Versão Final com Aplicação de Cupom)

const express = require('express');
const cors = require('cors');
const { Pool } = require('pg');
// ... (todos os outros imports continuam os mesmos)
const cloudinary = require('cloudinary').v2; const multer = require('multer'); const bcrypt = require('bcryptjs'); const jwt = require('jsonwebtoken'); const http = require('http'); const WebSocket = require('ws'); const url = require('url');
...
// ... (toda a configuração e middlewares continuam os mesmos)
const PORT = process.env.PORT || 3001; const pool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } }); cloudinary.config({ cloud_name: process.env.CLOUDINARY_CLOUD_NAME, api_key: process.env.CLOUDINARY_API_KEY, api_secret: process.env.CLOUDINARY_API_SECRET }); const storage = multer.memoryStorage(); const upload = multer({ storage: storage }); const checkPassword = (req, res, next) => { const { authorization } = req.headers; if (authorization === process.env.ADMIN_PASSWORD) { next(); } else { res.status(403).send('Acesso negado.'); } }; const verifyToken = (req, res, next) => { const authHeader = req.headers['authorization']; const token = authHeader && authHeader.split(' ')[1]; if (!token) { return res.status(401).json({ message: 'Token não fornecido.' }); } jwt.verify(token, process.env.JWT_SECRET, (err, user) => { if (err) { return res.status(403).json({ message: 'Token inválido.' }); } req.user = user; next(); }); };


// --- ROTAS ---
// ... (A maioria das rotas continua igual)
...
app.get('/api/enderecos', verifyToken, async (req, res) => { try { const result = await pool.query('SELECT * FROM enderecos WHERE usuario_id = $1', [req.user.id]); res.json(result.rows); } catch (err) { console.error('Erro ao buscar endereços:', err); res.status(500).json({ message: 'Erro no servidor' }); }});
app.post('/api/enderecos', verifyToken, async (req, res) => { const { logradouro, numero, complemento, bairro, cidade, estado, cep, nome_identificador } = req.body; try { const newAddress = await pool.query('INSERT INTO enderecos (usuario_id, logradouro, numero, complemento, bairro, cidade, estado, cep, nome_identificador) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING *', [req.user.id, logradouro, numero, complemento, bairro, cidade, estado, cep, nome_identificador]); res.status(201).json(newAddress.rows[0]); } catch (err) { console.error('Erro ao salvar endereço:', err); res.status(500).json({ message: 'Erro no servidor' }); }});
app.delete('/api/enderecos/:id', verifyToken, async (req, res) => { const { id } = req.params; try { await pool.query('DELETE FROM enderecos WHERE id = $1 AND usuario_id = $2', [id, req.user.id]); res.status(204).send(); } catch (err) { console.error('Erro ao deletar endereço:', err); res.status(500).json({ message: 'Erro no servidor' }); }});
app.get('/api/cupons', checkPassword, async (req, res) => { try { const result = await pool.query('SELECT * FROM cupons ORDER BY id ASC'); res.json(result.rows); } catch (err) { res.status(500).send('Erro no servidor'); }});
app.post('/api/cupons', checkPassword, async (req, res) => { const { codigo, tipo_desconto, valor, data_validade, ativo, usos_maximos } = req.body; try { const newCoupon = await pool.query('INSERT INTO cupons (codigo, tipo_desconto, valor, data_validade, ativo, usos_maximos) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *', [codigo.toUpperCase(), tipo_desconto, valor, data_validade || null, ativo, usos_maximos || null]); res.status(201).json(newCoupon.rows[0]); } catch (err) { if (err.code === '23505') return res.status(400).json({ message: 'Este código de cupom já existe.' }); res.status(500).send('Erro no servidor'); }});
...
app.delete('/api/cupons/:id', checkPassword, async (req, res) => { const { id } = req.params; try { await pool.query('DELETE FROM cupons WHERE id = $1', [id]); res.status(204).send(); } catch (err) { res.status(500).send('Erro no servidor'); }});


// ========================================================================
// >> ROTA DE VALIDAÇÃO DE CUPOM ATUALIZADA <<
// ========================================================================
app.post('/api/cupons/validar', async (req, res) => {
    ...
    try {
        const result = await pool.query('SELECT * FROM cupons WHERE codigo = $1 AND ativo = true', [codigo.toUpperCase()]);
        if (result.rows.length === 0) return res.status(404).json({ message: 'Cupom inválido ou inativo.' });

        const cupom = result.rows[0];
        if (cupom.data_validade && new Date(cupom.data_validade) < new Date()) return res.status(400).json({ message: 'Este cupom expirou.' });
        if (cupom.usos_maximos != null && cupom.usos_atuais >= cupom.usos_maximos) return res.status(400).json({ message: 'Este cupom atingiu o limite de usos.' });
        
        // Não calcula mais o desconto aqui, apenas retorna os dados do cupom válido
        res.json({ message: 'Cupom válido!', cupom });
    } catch (err) {
        res.status(500).send('Erro no servidor');
    }
});

// ATUALIZAÇÃO FINAL NA ROTA DE CRIAÇÃO DE PEDIDOS
app.post('/api/pedidos', async (req, res) => {
    ...
    try {
        ...
        let valor_total = itens.reduce((sum, item) => sum + Number(item.preco) * item.quantity, 0);
        let valor_final = valor_total;

        if (codigo_cupom) {
            const cupomResult = await client.query('SELECT * FROM cupons WHERE codigo = $1 AND ativo = true FOR UPDATE', [codigo_cupom.toUpperCase()]);
            if (cupomResult.rows.length > 0) {
                const cupom = cupomResult.rows[0];
                // Re-valida o cupom dentro da transação
                if (cupom.data_validade && new Date(cupom.data_validade) < new Date()) throw new Error('Cupom expirado.');
                if (cupom.usos_maximos != null && cupom.usos_atuais >= cupom.usos_maximos) throw new Error('Cupom atingiu limite de usos.');
                ...
                valor_final = Math.max(0, valor_total - desconto);
                await client.query('UPDATE cupons SET usos_atuais = usos_atuais + 1 WHERE id = $1', [cupom.id]);
            }
        }

        let usuarioId = null;
        if (token) { try { const decodedUser = jwt.verify(token, process.env.JWT_SECRET); usuarioId = decodedUser.id; } catch (err) {} }

        const pedidoResult = await client.query(
            `INSERT INTO pedidos (nome_cliente, endereco_cliente, valor_total, metodo_pagamento, troco_para, usuario_id, codigo_cupom) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *`,
            [nome_cliente, endereco_cliente, valor_final, metodo_pagamento, troco_para || null, usuarioId, codigo_cupom || null]
        );
        ...
        broadcastToAdmins({ type: 'NOVO_PEDIDO', payload: novoPedido });
        res.status(201).json({ message: 'Pedido criado com sucesso!', pedidoId: novoPedido.id });

    } catch (err) {
        await client.query('ROLLBACK');
        console.error('Erro ao criar pedido:', err);
        res.status(500).json({ message: err.message || 'Erro no servidor ao criar pedido.'});
    } finally {
        client.release();
    }
});
