// Arquivo: backend/index.js (Versão 12 - Com Lógica de Favoritos)
...
// ========================================================================
// >> ROTA DE PRODUTOS ATUALIZADA PARA INCLUIR FAVORITOS <<
// ========================================================================
app.get('/api/produtos', async (req, res) => {
    ...
    // A query agora verifica se um produto está na tabela de favoritos do usuário logado
    let query = `
        SELECT p.*, 
               array_agg(c.nome) as categorias,
               CASE WHEN f.produto_id IS NOT NULL THEN true ELSE false END as is_favorito
        FROM produtos p
        LEFT JOIN produto_categoria pc ON p.id = pc.produto_id
        LEFT JOIN categorias c ON pc.categoria_id = c.id
        LEFT JOIN favoritos f ON p.id = f.produto_id AND f.usuario_id = $1
    `;
    const params = [userId];
    ...
    if (categoria) {
        ...
        conditions.push(`p.id IN (SELECT produto_id FROM produto_categoria WHERE categoria_id = (SELECT id FROM categorias WHERE nome = $${params.length}))`);
    }
    ...
    query += ' GROUP BY p.id, f.produto_id ORDER BY p.id ASC';
...
// ========================================================================
// >> NOVAS ROTAS PARA GERENCIAR FAVORITOS (protegidas) <<
// ========================================================================
app.get('/api/meus-favoritos', verifyToken, async (req, res) => {
    try {
        const result = await pool.query('SELECT produto_id FROM favoritos WHERE usuario_id = $1', [req.user.id]);
        res.json(result.rows.map(row => row.produto_id)); // Retorna um array de IDs: [1, 5, 12]
    } catch (err) {
...
app.post('/api/favoritos', verifyToken, async (req, res) => {
    const { produtoId } = req.body;
    try {
        await pool.query('INSERT INTO favoritos (usuario_id, produto_id) VALUES ($1, $2)', [req.user.id, produtoId]);
...
app.delete('/api/favoritos/:produtoId', verifyToken, async (req, res) => {
    const { produtoId } = req.params;
    try {
        await pool.query('DELETE FROM favoritos WHERE usuario_id = $1 AND produto_id = $2', [req.user.id, produtoId]);
        res.status(200).json({ message: 'Produto desfavoritado com sucesso.' });
    } catch (err) {
