// Arquivo: backend/index.js (Com notificação corrigida)

// ... (todo o início do arquivo, imports, configs, etc. continua igual) ...
const express = require('express'); const cors = require('cors'); const { Pool } = require('pg'); const cloudinary = require('cloudinary').v2; const multer = require('multer'); const bcrypt = require('bcryptjs'); const jwt = require('jsonwebtoken'); const http = require('http'); const WebSocket = require('ws');
const app = express(); app.use(cors()); app.use(express.json());
const PORT = process.env.PORT || 3001; const pool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });
...
// ... (todas as outras rotas continuam iguais) ...
...
app.post('/api/pedidos', async (req, res) => {
  ...
  try {
    ...
    // Este loop é apenas para salvar no banco. Os itens já estão na variável 'itens' do req.body
    ...
    // >>>>> A CORREÇÃO <<<<<
    // Adiciona a lista de itens ao objeto do pedido ANTES de enviá-lo pela notificação
...
// ... (o resto do arquivo, com a lógica do WebSocket e o server.listen, continua igual)
