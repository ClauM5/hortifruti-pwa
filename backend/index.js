...
app.get('/api/meus-pedidos', verifyToken, async (req, res) => {
    try {
        ...
        const itensResult = await pool.query(
            'SELECT ip.*, p.nome as produto_nome FROM itens_pedido ip JOIN produtos p ON ip.produto_id = p.id WHERE ip.pedido_id = ANY($1::int[])',
            [pedidoIds]
        );
...
app.get('/api/admin/stats', checkPassword, async (req, res) => {
    ...
    try {
        const totalPedidosQuery = `SELECT COUNT(*) FROM pedidos ${whereClause}`;
        const receitaQuery = `SELECT SUM(valor_total) FROM pedidos WHERE status = 'Entregue' ${whereClause ? `AND data_pedido BETWEEN $1 AND $2` : ''}`;
        const pendentesQuery = `SELECT COUNT(*) FROM pedidos WHERE status NOT IN ('Entregue', 'Cancelado') ${whereClause ? `AND data_pedido BETWEEN $1 AND $2` : ''}`;
        
        const totalPedidosResult = await pool.query(totalPedidosQuery, params);
        const receitaResult = await pool.query(receitaQuery, params);
        const pendentesResult = await pool.query(pendentesQuery, params);
        ...
        res.json({
            totalPedidos: totalPedidosResult.rows[0].count || 0,
            receitaTotal: receitaResult.rows[0].sum || 0,
            pedidosPendentes: pendentesResult.rows[0].count || 0,
