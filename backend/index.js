// Arquivo: backend/index.js (Versão 13 - Com Rastreamento em Tempo Real)
...
const url = require('url'); // << Novo import para ler a URL da conexão
...
// ... (GET /produtos, POST /auth/register, POST /auth/login, etc. continuam aqui)
...
// ========================================================================
// >> ROTA DE ATUALIZAÇÃO DE STATUS ATUALIZADA PARA NOTIFICAR O CLIENTE <<
// ========================================================================
app.put('/api/pedidos/:id/status', checkPassword, async (req, res) => {
    ...
    try {
        ...
        // Se o pedido tiver um usuário associado, tenta notificá-lo
...
// Agora, em vez de um Set simples, usamos um Map para associar userId a uma conexão
    ...
    if (token) { // É um cliente logado
        jwt.verify(token, process.env.JWT_SECRET, (err, user) => {
            if (err) {
                ...
                userConnections.set(user.id, ws); // Associa o ID do usuário à conexão
                ws.userId = user.id; // Guarda o ID na própria conexão para fácil acesso
            }
        });
    } else { // É um admin (sem token)
        console.log('Novo admin conectado ao WebSocket.');
...
function sendMessageToUser(userId, message) {
    ...
    if (connection && connection.readyState === WebSocket.OPEN) {
        console.log(`Enviando mensagem para o usuário ${userId}:`, message);
        connection.send(JSON.stringify(message));
    }
}

// Função para enviar mensagem para todos os admins
function broadcastToAdmins(message) {
    const data = JSON.stringify(message);
    console.log('Enviando mensagem para todos os admins:', data);
